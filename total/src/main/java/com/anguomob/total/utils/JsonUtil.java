...
public class JsonUtil {
    ...
    public String getAssetsJson(String fileName) {
        ...
        try {
            AssetManager assetManager = AnguoUtils.getContext().getAssets();
     ...
     */
    @SuppressWarnings("unchecked")
    public static <T> ArrayList<T> jsonToObjectArraay(String jsonString, Class<T> clazz) {
        ArrayList<T> detail = new ArrayList<>();
        try {
            JSONArray data = new JSONArray(jsonString);
            for (int i = 0; i < data.length(); i++) {
//                CityJson entity = gson.fromJson(data.optJSONObject(i).toString(), CityJson.class);
                T entity = jsonToObject(data.optJSONObject(i).toString(), clazz);
                detail.add(entity);
            }
            return detail;
